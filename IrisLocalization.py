...
def find_pupil_later(img):
    ...
    circles = cv2.HoughCircles(thresholded_image, cv2.HOUGH_GRADIENT, dp=0.9, minDist=450, param1=1000, param2=15,
    ...
    if circles is not None:
        ...
        if gray_image2 is not None:
            circles2 = cv2.HoughCircles(gray_image2, cv2.HOUGH_GRADIENT, dp=0.8, minDist=450, param1=30, param2=30,
    ...
    for y in range(img.shape[0]):
        for x in range(img.shape[1]):
            if(big_circle is not None ):
                if not (is_between_circles(x,y,small_circle,big_circle) ):
